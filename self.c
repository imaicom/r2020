...
//	digitalWrite(24,0);
    ...
    if(digitalRead(15)==0) {
        ...
        if(digitalRead(15)!=0) {  // 瞬間押しの場合は、自動モードに入る.
			...
			digitalWrite(25,0); delay(100); // LED点滅
			...
			// 1回転目逆転テスト
			softPwmWrite(29,20); delay(600);	// 安定系
			while(digitalRead(4));
			softPwmWrite(29,0); delay(1000); if(digitalRead(10)==0) goto restart;
			softPwmWrite(29,10); delay(600);	// 正立
			while(digitalRead(4));
			softPwmWrite(29,0); delay(1000);	if(digitalRead(10)==0) goto restart;
...
/*
            ...
            while(read_file("cntWheel")<=130) { // 130カウントでセンサ検出走行終了
				...
				speed(20,20); // 両車輪前
				if(sensor==  1) speed(20,-10); // WWW W WWB
...
void main() {
    ...
    softPwmCreate( 1,0,20); // motor-3 20ms   // 大回転モータ正転
    softPwmCreate(29,0,20); // motor-3 20ms   // 大回転モータ逆転
